...
void DSP_AHRS_EKF_FilterUpdate_f32(DSP_AHRS_EKF_Instance_f32* filter, DSP_AHRS_DataInstance_f32* data, float dt)
{
    ...
    */
    ...
    // clang-format off
    float W[4 * 3] = {
       -dt_2 * data->AttitudeEstimate.i, -dt_2 * data->AttitudeEstimate.j, -dt_2 * data->AttitudeEstimate.k,
        dt_2 * data->AttitudeEstimate.r, -dt_2 * data->AttitudeEstimate.k,  dt_2 * data->AttitudeEstimate.j,
        dt_2 * data->AttitudeEstimate.k,  dt_2 * data->AttitudeEstimate.r, -dt_2 * data->AttitudeEstimate.i,
       -dt_2 * data->AttitudeEstimate.j,  dt_2 * data->AttitudeEstimate.i,  dt_2 * data->AttitudeEstimate.r
    };
    // clang-format on

    float Q[4 * 4];
    DSP_Matrix_SandwichMultiply_f32(Q, W, filter->GyroNoise, 4, 3);

    // P_t = F * P * F^T + Q
    float P_hat[16];
    DSP_Matrix_SandwichMultiply_f32(P_hat, F, filter->_P, 4, 4);
    DSP_Matrix_AddInplace_f32(P_hat, Q, 4, 4);
