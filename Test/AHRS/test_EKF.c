...
START_TEST(DSP_AHRS_EKF)
{
    ...
    float process_noise[3]     = {0.1f, 0.1f, 0.1f};  // Process noise covariance
    float measurement_noise[3] = {0.1f, 0.1f, 0.1f};  // Measurement noise covariance
    DSP_AHRS_EKF_Init_f32(&ekf_filter, process_noise, measurement_noise, NULL);

    DSP_AHRS_DataInstance_f32 data;
    DSP_AHRS_DataInit_f32(&data);

    FILE* fw = fopen("./matlab/data/ahrs_ekf_output.csv", "w");
    ...
    ck_assert_msg(1, "EKF AHRS output saved to ./matlab/data/ahrs_ekf_output.csv");
}
