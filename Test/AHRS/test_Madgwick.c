...
START_TEST(synth_data)
{
    ...
    const char* output_path = "./python/data/ahrs_madgwick_synth_output.csv";
    ...
    ck_assert_msg(passed, "Madgwick AHRS output saved to %s", full_out_path);
}
...
START_TEST(RepoIMU_data)
{
    ...
    const char* output_path = "./python/data/ahrs_madgwick_RepoIMU_output.csv";
    ...
    ck_assert_msg(1, "Madgwick AHRS output saved to %s", full_out_path);
}

__attribute__((constructor)) void register_madgwick_tests()
{
    tr_add_test("AHRS", "Madgwick", synth_data);
    tr_add_test("AHRS", "Madgwick", RepoIMU_data);
}